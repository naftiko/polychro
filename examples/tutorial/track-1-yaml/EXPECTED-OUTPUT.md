# Track 1 — YAML / OpenAPI — Expected output (verified against beta5)

> **STATUS: VERIFIED against `polychro 1.0.0-beta5`.** Every row below was
> re-verified by running the bundled `1.0.0-beta5` CLI from this directory on
> 2026-09-03 (originally verified against `1.0.0-beta1-SNAPSHOT` on 2026-06-26).
> One step still does **not** behave as the tutorial narrative intends — it is
> recorded here faithfully and tracked as a **known limitation** (see
> [`LIMITATIONS.md`](./LIMITATIONS.md), point 5). Nothing is "fixed" in this
...
| 4 | `polychro lint --config .polychro.yml step-4-openapi.yml` | 2 | `ERROR at $.info.version [type]: $.info.version: number found, string expected` + the `non-string-key` WARN. The `config.json-schema` block in `.polychro.yml` is consumed and the JSON-Schema stage runs as the narrative intends (this was LIMITATIONS §6, **resolved** — re-verified on `1.0.0-beta5`). |
...
| 7 | `polychro lint --format agent step-7-openapi.yml` | 1 | AgentFormatter JSON with one warning: `{"diagnostics":[{"severity":"warning","rule":"non-string-key","message":"Non-string YAML key: 200"}],"summary":{"errors":0,"warnings":1,"info":0},"tokens":52}` (pretty-printed by the CLI). Intended: clean (exit 0), but the `non-string-key` false positive applies here too. |
...
## Engine facts confirmed by running (beta1 2026-06-26, re-verified beta5 2026-09-03)
...
- **One divergence from the intended narrative** remains a known limitation, not an engine
  fact to rely on: the `non-string-key` false positive on quoted status codes
  ([`LIMITATIONS.md`](./LIMITATIONS.md) §5). The Step 4 `config.json-schema` block
  (formerly §6) is **resolved** — re-verified against `1.0.0-beta5` on 2026-09-03.
